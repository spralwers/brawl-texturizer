...
namespace SSBBTextures {
	public class TextureInfo {
		...
		internal static Texture[] ToTextureArray(string characterCode, string path, TextureInfo[] infos) {
			...
			foreach (TextureInfo info in infos) {
				if (info != null) {
					...
					try {
						byte[] pcsData = Util.ReadBytesFromFile(Path.Combine(path, string.Format("Fit{0}{1:00}.pcs", characterCode, info.Index)));
						byte[] pacData = Util.ReadBytesFromFile(Path.Combine(path, string.Format("Fit{0}{1:00}.pac", characterCode, info.Index)));
						texture=new Texture();
